...
@if(session("alerta_confirmar_render"))
    <script>
        Swal.fire({
            ...
            imageUrl: "{{ asset('assets/images/icone.png') }}",
            imageHeight: 150,
            title: "<label class='d-grid gap-3 py-2'><span class='{{ session('tema') == 'escuro' ? 'titulo_escuro cor_fonte_escuro' : 'titulo_claro cor_fonte_claro' }}'>{{ session('alerta_confirmar_render.titulo') }}</span><span class='{{ session('tema') == 'escuro' ? 'cor_fonte_escuro' : 'cor_fonte_claro' }} fs-5'>{{ session('alerta_confirmar_render.texto') }}</span></label>",
